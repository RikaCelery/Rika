...
object AdvanceMute : RegexCommand(
    ...
    suspend fun GroupMessageEvent.muteMember(eventMatchResult: EventMatchResult): ExecutionResult {
        ...
        if (member == "list") {
            group.members.filter { it.isMuted }
        ...
        //获取禁言对象(可能有多个)
        val targets = if (Regex("(all)|(全体(成员)?)").matches(member.split(' ').first()))
            null
        else if (message.filterIsInstance<At    >().isNotEmpty()) {
            //获取成员,过滤机器人自己and管理员
            message.filterIsInstance<At>().mapNotNull { group.members[it.target] }
