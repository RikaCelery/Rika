...
//                checkNotNull(targetIEntry) { "can not find entry where value=" + member.id + " in marry map." }
            ...
            return newMarry(from = sender, target = member as Member)
        }
        // 没有指定娶人
        ...
        return newMarry(sender, member1)

    }

    private suspend fun GroupMessageEvent.newMarry(from: Member, target: Member): ExecutionResult.Success {
